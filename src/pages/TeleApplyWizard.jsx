...
// 변경 요약
// - 기능 선택 라디오(택1): 'voice' 또는 'voice+sign'
// - 예약 성공 시 /reservation/confirm/:reservationId 로 이동 (state에 옵션/메모 포함)
// - 시간 포맷 HH:mm:ss 보정, 디버그 로그/에러 처리, 메모 sessionStorage 백업
...
export default function TeleApplyWizard() {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      console.error("[API ERROR] save residentId", e?.response?.status, e?.response?.data);
    ...
    try {
      ...
      console.error("[API ERROR] create reservation", e?.response?.status, e?.response?.data, "payload=", payload);
      alert(e?.response?.data?.message ?? `예약 생성 실패 (${e?.response?.status ?? "network"})`);
      return;
    }
