...
function createPeer(localStream, iceServers, { onTrack, onIce, onDataChannel }) {
  const pc = new RTCPeerConnection({
    iceServers: iceServers?.length ? iceServers : [{ urls: "stun:stun.l.google.com:19302" }, { urls: "stun:stun1.l.google.com:19302" }],
  });
...
export default function WebRtcSession() {
  ...
  const handleRecognizedSentence = (text) => { if (text) setRecognizedText(text); };
  ...
  const handleSendPatientCaption = async () => {
    ...
    try {
      await sendSignTextToDB(roomId, text);
  ...
  async function openCam(currentRole) {
    ...
    if (v) { v.srcObject = s; v.muted = true; try { await v.play(); } catch { /* noop */ } }
  }
  ...
  const sendCaption = useCallback((source, text) => {
    ...
    try { if (ch && ch.readyState === "open") ch.send(JSON.stringify(payload)); } catch (e) { console.debug("[DC] send error:", e); }
  }, []);
  ...
  async function startCall() {
    ...
    if (!peersRef.current.has(fixedKey)) {
      const pc = createPeer( localStreamRef.current, iceServers, {
        ...
        onIce: (ice) => {
          // [수정] 첫 번째 ICE candidate 객체를 콘솔에 한 번만 출력합니다.
  ...
  const toggleDoctorSTT = useCallback(() => {
    ...
    if (sttOn) {
      ...
      try { mediaRecRef.current?.rec?.stop(); } catch { /* noop */ }
      setSttOn(false);
    } else {
      try {
        ...
        if (!stream || stream.getAudioTracks().length === 0) { console.warn("녹음을 위한 오디오 트랙이 없습니다."); return; }
        const rec = new MediaRecorder(stream, { mimeType: "audio/webm" });
        mediaRecRef.current = { rec, chunks: [] };
        rec.ondataavailable = (e) => { if (e.data.size > 0) mediaRecRef.current.chunks.push(e.data); };
        rec.onstop = async () => {
          const audioBlob = new Blob(mediaRecRef.current.chunks, { type: "audio/webm" });
          if (audioBlob.size > 1000) {
            try {
              await sendSpeechToDB(roomId, audioBlob);
    ...
    <div className="visit">
      ...
      <main className="visit__main">
        ...
        <div className="tele__toolbar">
          <div className="tele__room"><label className="tele__label">예약번호</label><input className="tele__input" placeholder="예: 123" value={reservationId} onChange={(e) => setReservationId(e.target.value)} /></div>
          <div className="tele__actions"><button className="btn btn--ghost" onClick={() => joinAs("patient")}>환자로 참가</button><button className="btn btn--ghost" onClick={() => joinAs("doctor")}>의사로 참가</button><button className="btn btn--primary" onClick={startCall}>진료 시작</button><button className="btn" onClick={endCall}>종료</button></div>
        ...
        <div className="tele__videos">
          <section className="tele__panel">
            ...
            <div className="tele__video__frame">
              ...
              {role === "ROLE_PATIENT" && (<HandPoseTracker onSentence={handleRecognizedSentence} onLive={handleLiveWord} live={true} />)}
              {role === "ROLE_PATIENT" && (<div className="tele__live_status">{liveSignWord ? `인식중: ${liveSignWord}` : '수어 인식 대기 중...'}</div>)}
            </div>
            <div className="tele__caption">{leftCaption || "\u00A0"}</div>
          </section>
          <section className="tele__panel">
            <header className="tele__panel__title">상대 화면</header>
            <div className="tele__video__frame"><video ref={remoteVideoRef} autoPlay playsInline style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "12px" }} /></div>
            <div className="tele__caption">{rightCaption || "\u00A0"}</div>
          </section>
        </div>
        {role === "ROLE_PATIENT" && (
          <div className="tele__patient__input_area">
            <input type="text" className="tele__input" placeholder="이곳에 번역된 수어 문장이 표시됩니다." value={recognizedText} onChange={(e) => setRecognizedText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendPatientCaption()} />
            <button className="btn btn--primary" onClick={handleSendPatientCaption}> 자막 전송 </button>
          </div>
        )}
        <div className="tele__text__toolbar">
          {role === "ROLE_DOCTOR" && (<button className={`btn ${sttOn ? "btn--primary" : "btn--ghost"}`} onClick={toggleDoctorSTT}>{sttOn ? "음성인식 중지" : "음성인식 시작"}</button>)}
        </div>
      </main>
    </div>
  );
